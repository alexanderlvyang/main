...
<html>
  ...
  <header>
  	<p>子分类管理</p>
  	<a href='secondChildCategoryManager.do?categoryid=<%=request.getAttribute("grandFatherId") %>' id='return'>返回</a>
  </header>
    <div>
    	<table border=2>
    		<%
    		...
    		<%-- <td><a href='childCategoryManager.do?categoryid=<%=categoryList.get(i).getCategory_id()%>'>查看子分类</a></td> --%>
    		<td><a href="javascript:void(0)" class='delete' onclick='deleteCategory(id)' id='<%=categoryList.get(i).getCategory_id() %>'>删除</a></td>
    		</tr>
    		<%} %>
    	</table>
    </div>
    	<a href='addThirdCategory.do?categoryFatherId=<%=categoryFatherId %>' id='addnewchildcategory'>添加新分类</a>
  </body>
</html>
